...
# Core paths
DB_PATH = APP_DIR / "applyassist.db"
...
def load_profile() -> dict:
    """Load user profile from ~/.applyassist/profile.json."""
    import json
    if not PROFILE_PATH.exists():
        raise FileNotFoundError(
            f"Profile not found at {PROFILE_PATH}. Run `applyassist init` first."
        )
    return json.loads(PROFILE_PATH.read_text(encoding="utf-8"))


def load_search_config() -> dict:
    """Load search configuration from ~/.applyassist/searches.yaml."""
    import yaml
...
DEFAULTS = {
    ...
    "apply_timeout": 300,
    "viewport": "1280x900",
    # Human-pace guard: stop after this many applications prepared/submitted in a
    # rolling 24h window. Quality over volume — recruiters down-rank spray-applies
    # and high-volume bursts get accounts flagged. Override with APPLYASSIST_DAILY_CAP.
    "daily_cap": int(os.environ.get("APPLYASSIST_DAILY_CAP", "30")),
}


def load_env():
    """Load environment variables from ~/.applyassist/.env if it exists."""
...
# Tiers describe capability unlocked by locally-installed dependencies. ALL tiers
# are free in the open-source, self-hosted tool — this is not a paywall. The paid
# offering (see README "Open source vs hosted") is managed hosting, not these
# features. Auto-apply remains opt-in (--autopilot) regardless of tier.
TIER_LABELS = {
    1: "Discovery",
    2: "AI Scoring & Tailoring",
    3: "Assisted Apply (review mode)",
}
...
def get_tier() -> int:
    ...
    Tier 3 (Assisted Apply):           + Claude Code CLI + Chrome
    """
    load_env()
...
def check_tier(required: int, feature: str) -> None:
    ...
    if required >= 2 and not any(os.environ.get(k) for k in ("GEMINI_API_KEY", "OPENAI_API_KEY", "LLM_URL")):
        missing.append("LLM API key — run [bold]applyassist init[/bold] or set GEMINI_API_KEY")
    if required >= 3:
