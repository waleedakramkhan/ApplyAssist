"""ApplyAssist first-time setup wizard.

Interactive flow that creates ~/.applyassist/ with:
...
def _setup_profile() -> dict:
    ...
    console.print(Panel("[bold]Step 2: Profile[/bold]\nTell ApplyAssist about yourself. This powers scoring, tailoring, and auto-fill."))
    ...
    profile["personal"] = {
        ...
        "password": Prompt.ask("Job site password (used for login walls during assisted apply)", password=True, default=""),
    }
...
def _setup_searches() -> None:
    ...
    # Build YAML content
    lines = [
        "# ApplyAssist search configuration",
...
def _setup_ai_features() -> None:
    ...
    if not Confirm.ask("Enable AI scoring and resume tailoring?", default=True):
        console.print("[dim]Discovery-only mode. You can configure AI later with [bold]applyassist init[/bold].[/dim]")
        return
    ...
    env_lines = ["# ApplyAssist configuration", ""]
...
def _setup_auto_apply() -> None:
    """Configure assisted apply (requires Claude Code CLI)."""
    console.print(Panel(
        "[bold]Step 5: Assisted Apply (optional)[/bold]\n"
        "ApplyAssist can open a browser and fill out job applications for you,\n"
        "then STOP so you review and click Submit yourself (review mode).\n"
        "Uses Claude Code as the browser agent."
    ))

    if not Confirm.ask("Enable assisted apply?", default=True):
        console.print("[dim]You can still apply manually using the tailored resumes ApplyAssist generates.[/dim]")
        return
    ...
    if shutil.which("claude"):
        ...
        console.print(
            ...
            "Assisted apply won't work until Claude Code is installed."
        )

    # ApplyAssist does not solve CAPTCHAs by design — nothing to configure here.
    console.print(
        "\n[dim]Note: ApplyAssist never solves CAPTCHAs. If a site challenges you, "
        "it pauses and you finish that step yourself in the open browser.[/dim]"
    )
...
def run_wizard() -> None:
    ...
    console.print(
        Panel.fit(
            "[bold green]ApplyAssist Setup Wizard[/bold green]\n\n"
            "This will create your configuration at:\n"
            f"  [cyan]{APP_DIR}[/cyan]\n\n"
            "You can re-run this anytime with [bold]applyassist init[/bold].",
            border_style="green",
        )
    )
    ...
    from applyassist.config import get_tier, TIER_LABELS, TIER_COMMANDS
    ...
    unlock_hint = ""
    if tier == 1:
        unlock_hint = "\n[dim]To unlock Tier 2: configure an LLM API key (re-run [bold]applyassist init[/bold]).[/dim]"
    elif tier == 2:
